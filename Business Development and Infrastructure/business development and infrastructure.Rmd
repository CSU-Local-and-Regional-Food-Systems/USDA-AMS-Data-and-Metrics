...
We first import state and zip code level data to use in files where needed. 
...
Food hub data is provided in two different ways. First, the data will be provided as point level data. Second, it will be aggregated up to the county, state, and national number so we can get counts of food hubs per geography. 

We do not have county name in this data file. To get county name, we convert lat and long data using the lotlong2fips function developed by Loren Collingwood: https://rdrr.io/github/lorenc5/eiCompare/man/latlong2fips.html 

The following data is incorrect in the file: 
   * Incorrect address : Corbin Hill Food Project
   * ID 500057, Coulter Farms added address: 964 Price Rd, Honey Grove, PA 17035
   * ID 500060, Vermont Food Venture Center, zip code should be 05843
   * ID 500130, zip code was changed from 72334 to 72335.
   * Any row without a zip code it was added 
...
# Make point data long --- THIS NEEDS TO BE DONE AGAIN 
food_hub_point <- food_hub %>% select(!listing_id) %>% rename(
  business_name = listing_name, 
  business_address = location_address) %>% mutate(
    business_type = "food hub") %>% pivot_longer(
  cols = !c(lat, long), 
...
Get a shape file with county level data from the U.S. Census Bureau, Census Mapping Files, Cartographic Boundary Files. 

https://www.census.gov/geographies/mapping-files/time-series/geo/cartographic-boundary.html

Variables to keep - lat, long, establishment_name, address_line1, city_state, postal_code, inspection_activities, haccp_size, processing
...
# Make point data long ---THIS NEEDS TO CHANGE TO A NEW FORMAT
...
# Make point level data set ---UPDATE WITH NEW FORMAT
college_point <- college 
...
cold_capacity <- read_csv("data/rfwh0122/rfwh_p05_t001.csv", skip = 6) %>% 
  select(!c(`1`, u, `(number)_2`)) %>% rename(
    state_name = X3, 
    public_refrigerated_warehouses = `(number)`, 
    private_semi_private_refrigerated_warehouses = `(number)_1`) %>% filter(
...
cold_capacity <- cold_capacity %>% left_join(state_fips) %>% mutate(
  fips = ifelse(state_name=="United States", "US", fips))
